...
const AI_GATEWAY_FALLBACK_MODELS: Model<"anthropic-messages">[] = [
	{
...
function getAiGatewayFallbackModels(): Model<"anthropic-messages">[] {
	const existingModels = EXISTING_MODELS as Partial<Record<KnownProvider, Record<string, Model<"anthropic-messages">>>>;
	const existingAiGatewayModels = existingModels["vercel-ai-gateway"];
	if (existingAiGatewayModels) {
		return Object.values(existingAiGatewayModels).map((model) => ({
			...model,
			input: [...model.input],
			cost: { ...model.cost },
		}));
	...
	return AI_GATEWAY_FALLBACK_MODELS.map((model) => ({
		...model,
		input: [...model.input],
		cost: { ...model.cost },
	}));
...
async function fetchAiGatewayModels(): Promise<Model<any>[]> {
	try {
		...
		console.warn("Using fallback Vercel AI Gateway model metadata.");
		return getAiGatewayFallbackModels();
	}
}
